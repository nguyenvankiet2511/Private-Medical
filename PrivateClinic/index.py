...
from PrivateClinic import dao, login
...
# Phiếu khám
@app.route("/medical_examination_form", methods=['POST', 'GET'])
def create_medicalform():
    maBN = request.form.get('mab')
    mads = request.form.get('mads')
    benhNhan = dao.find_appointment(mab=maBN, mads=mads)
    medicine = dao.get_list_medicine()
    return render_template('create_medical_form.html', medicine=medicine, benhNhan=benhNhan)
...
def save_medical_form():
    ...
    maDS = request.form.get('ma_ds')
    dao.update_list_appointment(maBN=maBN, maDS=maDS)
...
def list_medical_examiniation():
    ...
    list_appointment = dao.get_list_medical_examination(bacSi_id, role)

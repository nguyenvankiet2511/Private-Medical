...
def get_history_medical_all():
    query = db.session.query(PhieuKham.id.label('id_pk'), Nguoi.id, Nguoi.hoTen, PhieuKham.ngayKham,
                             PhieuKham.trieuChung, PhieuKham.chuanDoan).join(
...
def get_medicine_report(id_pk):
    return db.session.query(Thuoc.gia, PhieuThuoc.soLuong, Thuoc.tenThuoc, PhieuThuoc.cachDung).join(PhieuThuoc, Thuoc.id.__eq__( PhieuThuoc.thuoc_id)).filter(PhieuThuoc.phieu_kham_id.__eq__(id_pk)).all()
...
# update ds dk
def update_list_appointment(maBN, maDS):
    result = ChiTietDanhSach.query.filter(ChiTietDanhSach.danhSachKham_id==maDS, ChiTietDanhSach.benhNhan_id==maBN).first()
...
def get_list_medical_examination(bacSi_id, role):
    if role == UserRole.DOCTOR:
        results = db.session.query(DanhSachKham.id, ChiTietDanhSach.benhNhan_id, ChiTietDanhSach.is_active,
            ...
            .join(BacSi, BacSi.id == ChiTietDanhSach.bacSi_id) \
            .join(Nguoi, BacSi.id == Nguoi.id) \
            .filter(ChiTietDanhSach.bacSi_id == bacSi_id) \
            .all()
        return results
    else:
        results = db.session.query(DanhSachKham.id, ChiTietDanhSach.benhNhan_id, ChiTietDanhSach.is_active,
...
def get_list_invoice():
    result = db.session.query(HoaDon.tienKham, Nguoi.hoTen, HoaDon.tienThuoc, HoaDon.ngayKham, PhieuKham.benh_nhan_id,
                              HoaDon.id, HoaDon.phieu_kham_id).join(PhieuKham,
                                                                    PhieuKham.id == HoaDon.phieu_kham_id).join(Nguoi,
...
def find_appointment(mab, mads):
    result = db.session.query(Nguoi.hoTen, ChiTietDanhSach.benhNhan_id, ChiTietDanhSach.danhSachKham_id, ChiTietDanhSach.is_active).join(Nguoi,
                                                                                                              Nguoi.id == ChiTietDanhSach.benhNhan_id).filter(
        ChiTietDanhSach.danhSachKham_id == mads, ChiTietDanhSach.benhNhan_id == mab).first()
    return result

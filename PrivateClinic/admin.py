from PrivateClinic import app, db, dao
...
admin = Admin(app=app, name='Quản Trị NWA', template_mode='bootstrap4', index_view=MyAdminIndex())
...
class StatsView(AuthenticatedView):
    ...
    def index(self):
        ...
        doanh_thu_thang = dao.tong_doanh_thu_theo_thang(year=year)
        session['tong_doanh_thu']=sum(doanh_thu_thang)
        session['tong_doanh_thu_thang'] = doanh_thu_thang[month-1]
        tan_suat_kham = dao.tan_suat_kham(year=year)
        session['so_luong_kham'] = tan_suat_kham[month - 1]
        doanh_thu_quy = dao.doanh_thu_theo_quy(year=year)
        ten_thuoc_list, so_luong_list = dao.get_usage_by_month_and_year(selected_month=month, selected_year=year)
        return self.render('admin/stats.html', tan_suat_kham=tan_suat_kham, doanh_thu_thang=doanh_thu_thang,
